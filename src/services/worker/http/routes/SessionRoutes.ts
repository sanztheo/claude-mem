...
export class SessionRoutes extends BaseRouteHandler {
  ...
  private getActiveAgent():
    ...
    if (isCodexSelected()) {
      if (isCodexAvailable()) {
        ...
        throw new Error(
          "Codex provider selected but not configured. Set CLAUDE_MEM_CODEX_AUTH_METHOD and (if api) CLAUDE_MEM_CODEX_API_KEY, or run `codex login` for CLI auth.",
        );
      }
    }
    ...
    session.generatorPromise = agent
      ...
      .catch((error) => {
        ...
        if (myController.signal.aborted) {
          logger.debug("HTTP", "Generator catch: ignoring error after abort", {
            sessionId: session.sessionDbId,
          });
          return;
        }
        ...
        // APPROVED OVERRIDE
        if (
          errorMsg.includes("code 143") ||
          errorMsg.includes("signal SIGTERM")
        ) {
          logger.warn(
            "SESSION",
            "Generator killed by external signal — aborting session to prevent respawn",
            {
              sessionId: session.sessionDbId,
              provider,
              error: errorMsg,
      ...
      .finally(async () => {
        ...
        if (!wasAborted) {
          ...
          if (pendingCount > 0) {
            ...
            if (!session.restartGuard)
              session.restartGuard = new RestartGuard();
            const restartAllowed = session.restartGuard.recordRestart();
            session.consecutiveRestarts =
              (session.consecutiveRestarts || 0) + 1; // Keep for logging

            if (!restartAllowed) {
              logger.error(
                "SESSION",
                `CRITICAL: Restart guard tripped — too many restarts in window, stopping to prevent runaway costs`,
                {
                  sessionId: sessionDbId,
                  pendingCount,
            ...
            const backoffMs = Math.min(
              1000 * Math.pow(2, session.consecutiveRestarts - 1),
              8000,
            );
  ...
  private handleObservationsByClaudeId = this.wrapHandler(
    (req: Request, res: Response): void => {
      ...
      try {
        sessionDbId = store.createSDKSession(
          contentSessionId,
          project,
          "",
          undefined,
   ...
   */
  private handleStatusByClaudeId = this.wrapHandler(
    (req: Request, res: Response): void => {
  ...
  private handleCompleteByClaudeId = this.wrapHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      if (!activeSession) {
        ...
        logger.debug(
          "SESSION",
          "session-complete: Session not in active map; continuing with DB-backed completion",
          {
            contentSessionId,
            sessionDbId,
          },
      ...
      res.json({
        status: activeSession ? "completed" : "completed_db_only",
        sessionDbId,
      });
    },
  );
  ...
  private applyTierRouting(
    ...
    const allSimple = pending.every(
      (m) =>
        m.message_type === "observation" &&
        m.tool_name &&
        SessionRoutes.SIMPLE_TOOLS.has(m.tool_name),
    );
