...
} from "./infrastructure/WorktreeAdoption.js";
...
export class WorkerService {
  ...
  constructor() {
    ...
    // Wire Claude SDK as fallback for Codex (mirrors Gemini/OpenRouter pattern)
    this.codexAgent.setFallbackAgent(this.sdkAgent);
    ...
    this.server.app.use("/api", async (req, res, next) => {
      ...
      try {
        ...
        if (error instanceof Error) {
          ...
          logger.error(
            "WORKER",
            `Request to ${req.method} ${req.path} rejected — DB not initialized with non-Error`,
            {},
  ...
  private async initializeBackground(): Promise<void> {
    try {
      ...
      let adoptions: Awaited<
        ReturnType<typeof adoptMergedWorktreesForAllKnownRepos>
      > | null = null;
      try {
        ...
        if (err instanceof Error) {
          logger.error(
            "WORKER",
            "Worktree adoption failed (non-fatal)",
            {},
            err,
          );
        } else {
          logger.error(
            "WORKER",
            "Worktree adoption failed (non-fatal) with non-Error",
            {},
      ...
      if (adoptions) {
        for (const adoption of adoptions) {
          if (
            adoption.adoptedObservations > 0 ||
            adoption.adoptedSummaries > 0 ||
            adoption.chromaUpdates > 0
          ) {
            logger.info(
              "SYSTEM",
              "Merged worktrees adopted on startup",
              adoption,
            );
          }
      ...
      const { SearchOrchestrator } =
        await import("./worker/search/SearchOrchestrator.js");
      ...
      this.server.registerRoutes(
        new CorpusRoutes(this.corpusStore, corpusBuilder, knowledgeAgent),
      );
      ...
      try {
        ...
        logger.warn(
          "WORKER",
          "MCP loopback self-check failed, cleaning up subprocess",
          {
            error:
        ...
        try {
          ...
          logger.debug(
            "WORKER",
            "transport.close() failed during MCP cleanup",
            {
              error:
                transportCloseError instanceof Error
                  ? transportCloseError.message
                  : String(transportCloseError),
            },
          );
        }
        logger.info(
          "WORKER",
          "Bundled MCP server remains available for external stdio clients",
          {
            path: mcpServerPath,
          },
        );
        ...
        async () => {
          try {
            ...
            if (e instanceof Error) {
              ...
              logger.error(
                "WORKER",
                "Stale session reaper error with non-Error",
                {},
                new Error(String(e)),
              );
            }
          ...
          try {
            ...
            const purged = pendingStore.clearFailedOlderThan(
              FAILED_MESSAGE_RETENTION_MS,
            );
            if (purged > 0) {
              logger.info(
                "SYSTEM",
                `Purged ${purged} stale failed pending messages (older than 1h)`,
              );
            }
          } catch (e) {
            if (e instanceof Error) {
              ...
              logger.error(
                "WORKER",
                "Failed message purge error with non-Error",
                {},
                new Error(String(e)),
              );
            }
          ...
          try {
            ...
            if (e instanceof Error) {
              ...
              logger.error(
                "WORKER",
                "WAL checkpoint error with non-Error",
                {},
                new Error(String(e)),
              );
            }
  ...
  private async startTranscriptWatcher(
    ...
    const transcriptsEnabled =
      settings.CLAUDE_MEM_TRANSCRIPTS_ENABLED !== "false";
    if (!transcriptsEnabled) {
      logger.info(
        "TRANSCRIPT",
        "Transcript watcher disabled via CLAUDE_MEM_TRANSCRIPTS_ENABLED=false",
      );
      return;
    }

    const configPath =
      settings.CLAUDE_MEM_TRANSCRIPTS_CONFIG_PATH || DEFAULT_CONFIG_PATH;
    ...
    const statePath = expandHomePath(
      transcriptConfig.stateFile ?? DEFAULT_STATE_PATH,
    );
    ...
    try {
      ...
      if (error instanceof Error) {
        logger.error(
          "WORKER",
          "Failed to start transcript watcher (continuing without Codex ingestion)",
          {
            configPath: resolvedConfigPath,
          },
          error,
        ...
        logger.error(
          "WORKER",
          "Failed to start transcript watcher with non-Error (continuing without Codex ingestion)",
          {
            configPath: resolvedConfigPath,
          },
          new Error(String(error)),
  ...
  private startSessionProcessor(
    ...
    session.generatorPromise = agent
      ...
      .catch(async (error: unknown) => {
        ...
        const staleResumePatterns = [
          "aborted by user",
          "No conversation found",
        ];
        if (
          staleResumePatterns.some((p) => errorMessage.includes(p)) &&
      ...
      .finally(async () => {
        ...
        if (pendingCount > 0) {
          ...
          if (!restartAllowed) {
            logger.error(
              "SYSTEM",
              "Restart guard tripped: too many restarts in window, stopping to prevent runaway costs",
              {
                sessionId: session.sessionDbId,
                pendingCount,
  ...
  private isSessionTerminatedError(error: unknown): boolean {
    ...
    return WorkerService.SESSION_TERMINATED_PATTERNS.some((pattern) =>
    ...
    if (isOpenRouterAvailable()) {
      try {
        ...
        if (e instanceof Error) {
          logger.error(
            "WORKER",
            "Fallback OpenRouter failed, will abandon messages",
            { sessionId: sessionDbId },
            e,
          );
        } else {
    ...
    if (staleSessionIds.length > 0) {
      ...
      try {
        ...
        if (error instanceof Error) {
          logger.error(
            "WORKER",
            "Failed to mark stale sessions as failed",
          ...
          logger.error(
            "WORKER",
            "Failed to mark stale sessions as failed with non-Error",
            { staleCount: ids.length },
            new Error(String(error)),
          );
        }
      ...
      try {
        ...
        if (error instanceof Error) {
          logger.error(
            "WORKER",
            "Failed to clean up stale pending messages",
            { staleCount: ids.length },
            error,
          );
        } else {
          logger.error(
            "WORKER",
            "Failed to clean up stale pending messages with non-Error",
            { staleCount: ids.length },
            new Error(String(error)),
          );
        }
    ...
    for (const sessionDbId of orphanedSessionIds) {
      ...
      try {
        ...
        if (error instanceof Error) {
          logger.error(
            "WORKER",
            `Failed to initialize/start session ${sessionDbId}`,
            { sessionDbId },
            error,
          );
        } else {
          logger.error(
            "WORKER",
            `Failed to initialize/start session ${sessionDbId} with non-Error`,
            { sessionDbId },
            new Error(String(error)),
          );
        }
...
async function main() {
  ...
  switch (command) {
    ...
    case "gemini-cli": {
      ...
      const geminiResult = await handleGeminiCliCommand(
        geminiSubcommand,
        process.argv.slice(4),
      );
    ...
    case "hook": {
      ...
      if (!platform || !event) {
        ...
        console.error(
          "Events: context, session-init, observation, summarize, session-complete, user-message",
        );
        process.exit(1);
      }
    ...
    case "adopt": {
      ...
      const branchValue =
        branchIndex !== -1 ? process.argv[branchIndex + 1] : undefined;
      if (
        branchIndex !== -1 &&
        (!branchValue || branchValue.startsWith("--"))
      ) {
        console.error(
          "Usage: adopt [--dry-run] [--branch <branch>] [--cwd <path>]",
        );
        process.exit(1);
      }
      ...
      console.log(
        `  Parent project:       ${result.parentProject || "(unknown)"}`,
      );
      console.log(`  Repo:                 ${result.repoPath}`);
      console.log(`  Worktrees scanned:    ${result.scannedWorktrees}`);
      console.log(
        `  Merged branches:      ${result.mergedBranches.join(", ") || "(none)"}`,
      );
      ...
      if (result.chromaFailed > 0) {
        console.log(
          `  Chroma sync failures: ${result.chromaFailed} (will retry on next run)`,
        );
      }
    ...
    default: {
      ...
      worker.start().catch(async (error) => {
        ...
        if (isPortConflict && (await waitForHealth(port, 3000))) {
          logger.info(
            "SYSTEM",
            "Duplicate daemon exiting — another worker already claimed port",
            { port },
          );
          process.exit(0);
        }
